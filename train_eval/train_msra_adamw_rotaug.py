...
from utils import rotate_point_cloud_by_angle_flip
...
parser.add_argument('--dataset_path', type=str, default = '../../HandFoldNew/data/msra_process_pure',  help='model name for training resume')
...
save_dir = os.path.join(opt.save_root_dir, 'msra_handgraf_adam_rotaug', subject_names[opt.test_index])
...
for epoch in range(opt.start_epoch, opt.nepoch):
	...
	for i, data in enumerate(tqdm(train_dataloader, 0)):
		...
		points, volume_length, _, gt_xyz = data
		...
		fold1, fold2, estimation = netR(points.transpose(1,2), points.transpose(1,2))
		loss = (criterion(estimation, gt_xyz)*1+criterion(fold1, gt_xyz)*0.25+criterion(fold2, gt_xyz)*0.25)*63
	...
	if test_best_error > test_wld_err:
		...
		torch.save(netR.state_dict(), '%s/best_model.pth' % (save_dir))
