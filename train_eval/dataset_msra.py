...
class HandPointDataset(data.Dataset):
    def __init__(self, root_path, opt, train=True):
        ...
        self.offset = self.offset.index_select(0, valid_ind.long())
    ...
    def __getitem__(self, index):
        return self.point_clouds[index, :, :], self.volume_length[index], self.offset[index, :], self.gt_xyz[index, :]
    ...
    def __loaddata(self, data_dir):
        ...
        offset = sio.loadmat(os.path.join(data_dir, "Volume_offset.mat"))
        ...
        self.offset[(self.start_index - 1):self.end_index, :] = offset['Volume_offset'].astype(np.float32)
